...
// console.log(parsed);
let inspections = {}
let highestDivisable = Object.values(parsed).reduce((acc, monkey) => acc *= monkey.test, 1);
 console.log(highestDivisable);
for (var i = 0; i < 10000; i++) {
  for (monkey of parsed) {
    ...
    for(item of monkey.startingItems) {
      ...
      let worryLevel = ((symbol == "+" ? item + val : symbol == "*" ? item * val : symbol == "-" ? item - val : symbol == "/" ? item / val : null))%highestDivisable;
...
console.log((Object.values(inspections).sort((a, b) => b-a).slice(0,2).reduce((a, b) => a * b, 1)));
